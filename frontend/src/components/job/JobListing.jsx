...
const JobListing = ({ filters }) => {
  ...
  const filteredJobs = jobs.filter((job) => {
    ...
    // Check if job matches the search, location, and job type filters
    const searchMatch = search
      ? job.jobTitle.toLowerCase().includes(search.toLowerCase())
      : true;
    ...
    // Convert the job's salary to thousands (K)
    const convertedJobSalary = convertSalaryToK(job.salaryRange);

    // Extract min and max salary from the filter range (salaryRange is an array [min, max])
